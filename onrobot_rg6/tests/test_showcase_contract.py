...
"""Validate the RG6 description and software-rendered showcase contract."""
...
XACRO_FILE = PACKAGE_DIR / 'urdf' / 'realmesh_onrobot_rg6.urdf.xacro'
RVIZ_FILE = PACKAGE_DIR / 'rviz' / 'rg6_ros2_control_showcase.rviz'
CONTROL_LAUNCH = PACKAGE_DIR / 'launch' / 'control.launch.py'


class Rg6ShowcaseContract(unittest.TestCase):
    ...
    def test_every_visual_mesh_is_an_installed_rg6_resource(self):
        ...
        for visual in visuals:
            ...
            self.assertTrue(
                resource.startswith('package://onrobot_rg6/meshes/')
            )
    ...
    def test_control_launch_supports_the_isaac_backend(self):
        """Forward RG6 joint topics, timeout, and simulation time."""
        launch = CONTROL_LAUNCH.read_text(encoding='utf-8')
        ...
        for argument in (
                ...
                'use_sim_time'):
            self.assertIn(f"DeclareLaunchArgument(\n            '{argument}'",
                          launch)
        self.assertIn("' isaac_joint_commands_topic:='", launch)
        self.assertIn("' isaac_joint_states_topic:='", launch)
        self.assertIn("' isaac_state_timeout_ms:='", launch)
